...
class LectureXml:
	...
	def __init__(self, nom, erreurLog,log):
		...
		try :
			ptrfic = open(nom+'.xml',encoding='utf-8')
			d = parse(ptrfic)
		except Exception as msg:
			#raise
			if self.erreurLog:
				 self.erreurLog.erreur(self.log,"__init__","Erreur de lecture du fichier : "+nom+".xml\n",-1,msg)
			else:
		...
		try :
			...
			if self.erreurLog:
				self.erreurLog.erreur(self.log,"__init__","Erreur d'analyse du fichier : "+nom+".xml\n",-1,msg)
			else:
		...
		for a in balise_pere:
			...
			try :
				...
				for noeud in tab_node :
					tableau.append(noeud.childNodes[0].data)
				resultat = tableau
			except Exception as msg:
	...
	def chargerListeAttribut(self,pere,fils,attribut):
		...
		try :
			...
			if self.erreurLog:
				...
				print("Erreur structure fichier XML vers <"+pere+ "> "+ msg)
			exit(0)
		# pour tous les elements sous le pere
		for a in balise_pere:
			#on recupere un element
